...
def get_TC_seasons(
    **kwargs,
    ...
    if tracks_path is None:
        raise ValueError("tracks_path argument must be provided")
        # print traceback
...
def get_timeseries_sets(splits: dict, season_dict: dict, **kwargs):
    ...
    datadir_path = kwargs.get("datadir", None)
    if datadir_path is None:
        raise ValueError("datadir argument must be provided")
        # print traceback
        print(traceback.format_exc())
...
def read_py_track_file(tracks_path=None, year=2000, backend=pd.read_csv):
    """
...
class AI:
    ...
    def animate_var(self, var, **kwargs):
        ...
        save_path = kwargs.get(
            "save_path",
            f"../dev/results/{self.__parent.uid}.{self.model}_{var}_animation.gif",
        )
...
class ReAnal:
    ...
    def animate_2d(self, **kwargs):
        ...
        save_path = kwargs.get(
            "save_path",
            f"../data/{self.__parent.name}_animation.gif",
        )
...
def compute_r2_by_lead_from_results(
    ...
    Assumes:
      - results_df has: ["SID", "Initial Time", "Valid Time", f"SE_{variable}"]
    ...
    se_col = f"SE_{variable}"
    gt_col = "USA_WIND" if variable == "wind" else "USA_PRES"

    need_results = {"SID", "Initial Time", "Valid Time", se_col}
    need_ib = {"SID", "ISO_TIME", "USA_WIND", "USA_PRES"}
    if not need_results.issubset(results_df.columns):
        missing = need_results - set(results_df.columns)
    ...
    # Results: coerce times and SE to numeric
    m = results_df.copy()
    ...
    # lead_hours (int) and hour key for Valid Time
    m["lead_hours"] = (
    ...
    # IBTrACS: filter year (optional), coerce GT numeric, hour key
    ib = ibtracs_df.copy()
