...
os.makedirs(save_dir + "result_export/", exist_ok=True)
EVAL_DIR = data_dir
...
try:
    ibtracs = toolbox.read_hist_track_file(
        tracks_path=os.path.join(os.curdir, "ibtracs")
    )
...
for fn in result_files:
    ...
    for base in NON_R2_BASES:
        ...
        agg = g.agg(["mean", "count", q25, q75, "std"]).sort_index()
        # unify column names
...
def _coverage_series_vs_ib_from_raw(
    ...
    for lh, grp in dfm.groupby("lead_hours"):
        ...
        cnt_vals[float(lh)] = len(keys)
        # print(len(keys), den)
        # if len(keys) > den:
        #     print(lh, len(keys), den)

    return pd.Series(cov_vals, dtype=float).sort_index(), pd.Series(
        cnt_vals, dtype=float
    ).sort_index()
    ...
    for c, base in enumerate(row):
        ...
        if base in ("R2_pressure", "R2_wind"):
            ...
            for fn in result_files:
                ...
                low = fn.lower()
                if any(tok in low for tok in ("ri")):
                    continue
        ...
        for forecast_label, agg in per_forecast.items():
            ...
            save_label = ["mean"]
            if not base.startswith("CRPS"):
                save_label.append("std")
            for save_type in save_label:
                if save_type == "mean":
                    y = y
                elif save_type == "std":
                    y = y_std
                try:
                    raw_df = pd.read_excel(
                        f"{save_dir}/result_export/raw_comparison.xlsx",
                        sheet_name=base + "_" + save_type,
                    )
                except:
                    raw_df = pd.DataFrame()
                col_name = label_txt
                ...
                if "$" in col_name:
                    col_name = col_name.replace("$\it{", "")
                    col_name = col_name.replace("}$", "")
                y_values = y[:20]
                raw_df[col_name] = y_values
                file_path = f"{save_dir}/result_export/raw_comparison.xlsx"
                if not os.path.exists(file_path):
                    # === 情况 A: 文件不存在 -> 直接保存 (自动新建) ===
                    # 注意：这里不需要用 ExcelWriter 的复杂模式，直接 to_excel 即可
                    raw_df.to_excel(
                        file_path, sheet_name=base + "_" + save_type, index=False
                    )
                else:
                    # === 情况 B: 文件已存在 -> 使用追加模式 ===
                    # mode='a' 允许在该文件中增加/修改 sheet，而不删除其他 sheet
                    # if_sheet_exists='replace' 表示如果该 sheet 已存在，就用新的数据覆盖它
                    with pd.ExcelWriter(
                        file_path,
                        engine="openpyxl",
                        mode="a",
                        ...
                        raw_df.to_excel(
                            writer, sheet_name=base + "_" + save_type, index=False
                        )
            # print(label_txt, x, y)
            # print("hblgzsx")
            try:
...
fig.savefig(
    f"{save_dir}/result_export/TCBench_raw_comparison.pdf",
    format="pdf",
    bbox_inches="tight",
)
plt.show()
...
# %% FAIR comparison figure and Coverage
...
if _persist_fair is not None:
    ...
    for f in os.listdir(EVAL_DIR):
        if not f.endswith("_results.csv"):
            continue
        low = f.lower()
        if any(tok in low for tok in ("persistence", "ri", "climatology")):
            continue
    ...
    for i, (base, title, unit) in enumerate(metrics_to_plot_fair):
        ...
        for label, dfm in overlays.items():
            ...
            if base.startswith("CRPS"):
                ...
                agg = sfilled.groupby("lead_hours")["filled"].mean().sort_index()
                agg_std = sfilled.groupby("lead_hours")["filled"].std().sort_index()
                try:
                    idx = agg.index.astype(float)
                except Exception:
                    idx = agg.index
                ...
                agg_std = (
                    pd.Series(agg_std.values, index=idx).reindex(allowed_leads).dropna()
                )
            plot_kwargs = dict(
                linestyle="-",
                marker="o",
            ...
            print(f"\n[DATA EXPORT] Metric: {base} | Model: {pretty_label}")
            # print(agg.to_string())
            # if "$" in pretty_label:
            #     pretty_label = pretty_label.replace("$\it{", "")
            #     pretty_label = pretty_label.replace("}$", "")

            # export_filename = f"export_{base}_{pretty_label}.csv"
            # agg.to_csv(export_filename)
            save_label = ["mean"]
            if not base.startswith("CRPS"):
                save_label.append("std")
            for save_type in save_label:
                try:
                    fair_df = pd.read_excel(
                        f"{save_dir}/result_export/fair_comparison.xlsx",
                        sheet_name=base + "_" + save_type,
                    )
                except:
                    fair_df = pd.DataFrame()
                col_name = pretty_label
                ...
                y_values = agg.values[:20]

                fair_df[col_name] = y_values
                file_path = f"{save_dir}/result_export/fair_comparison.xlsx"
                if not os.path.exists(file_path):
                    # === 情况 A: 文件不存在 -> 直接保存 (自动新建) ===
                    # 注意：这里不需要用 ExcelWriter 的复杂模式，直接 to_excel 即可
                    fair_df.to_excel(
    ...
    fig2.savefig(
        f"{save_dir}/result_export/TCBench_fair_comparison.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.show()
    ...
    for label, dfm in overlays.items():
        ...
        cov_series, cnt_series = _coverage_series_vs_ib_from_raw(dfm, ib_denom_cov)
        cov_series = cov_series.reindex(EXPLICIT_LEADS).astype(float)
        cnt_series = cnt_series.reindex(EXPLICIT_LEADS).astype(float)
        x = np.asarray(EXPLICIT_LEADS, dtype=float)
        y = cov_series.values
        y_cnt = cnt_series.values
        ...
        try:
            coverage_df = pd.read_excel(
                f"{save_dir}/result_export/TCBench_coverage.xlsx",
                sheet_name=col_name,
            )
            cnt_df = pd.read_excel(
                f"{save_dir}/result_export/TCBench_cnt.xlsx",
                sheet_name=col_name,
            )
        except:
        ...
        y_values = y
        coverage_df["lead_time"] = x
        coverage_df["coverage"] = y_values

        cnt_df["lead_time"] = x
        cnt_df["cnt"] = y_cnt

        file_path = f"{save_dir}/result_export/TCBench_coverage.xlsx"
        file_cnt_path = f"{save_dir}/result_export/TCBench_cnt.xlsx"
        if not os.path.exists(file_path):
            coverage_df.to_excel(file_path, sheet_name=col_name, index=False)
        else:
            with pd.ExcelWriter(
                file_path, engine="openpyxl", mode="a", if_sheet_exists="replace"
            ) as writer:
                coverage_df.to_excel(writer, sheet_name=col_name, index=False)
        if not os.path.exists(file_cnt_path):
            cnt_df.to_excel(file_cnt_path, sheet_name=col_name, index=False)
        else:
            with pd.ExcelWriter(
                file_cnt_path, engine="openpyxl", mode="a", if_sheet_exists="replace"
            ) as writer:
                cnt_df.to_excel(writer, sheet_name=col_name, index=False)
    ...
    fig_cov.savefig(
        f"{save_dir}/result_export/TCBench_coverage.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def _order_rows_fnv3_last(index_list):
    ...
    is_google = lambda n: (
        _is_google(n) or str(n).lower() == "fnv3" or "fnv3" in str(n).lower()
    )
    order = []
...
fig_prob.savefig(
    f"{save_dir}/result_export/TCBench_scorecard_probabilistic.pdf",
    format="pdf",
    bbox_inches="tight",
)
plt.show()
...
fig.savefig(
    f"{save_dir}/result_export/TCBench_scorecard_deterministic.pdf",
    format="pdf",
    bbox_inches="tight",
)
plt.show()
...
for fn in sorted(model_files):
    try:
        ...
        export_filename = f"./result_export/export_Fig10_RI_{label}.csv"
        curves[label].to_csv(export_filename)
