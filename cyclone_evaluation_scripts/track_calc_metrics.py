    ...
    for track_file in track_files:
        ...
        if _cols.duplicated().any():
            if verbose:
                ...
                print(
                    f"Warning: duplicated columns in {track_file}: {dups} — keeping first."
                )
        ...
        for metric in error_metrics:
            if verbose:
                ...
                print(f"Input shape: {track_df.shape}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_folder", type=str, default="./", help="evaluation folder"
    )
    parser.add_argument("--year", type=int, default=2023, help="year to evaluate")
    parser.add_argument(
        "--select_files", type=str, default=None, help="select files to evaluate"
    )
    parser.add_argument("--recompute", action="store_true", help="recompute results")
    parser.add_argument(
        "--only_ri", action="store_true", help="only compute RI metrics"
    )
    parser.add_argument("--verbose", action="store_true", help="verbose output")
    parser.add_argument("--save_results", action="store_true", help="save results")
    parser.add_argument(
        "--return_dataframes", action="store_true", help="return dataframes"
    )

    args = parser.parse_args()
    eval_folder = args.eval_folder
    ibtracs_dir = os.path.join(os.curdir, "ibtracs")
